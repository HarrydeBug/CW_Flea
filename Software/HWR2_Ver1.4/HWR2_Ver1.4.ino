...
#define tuneup A0       //pin number used for tuning up in freq
#define tunedn A1       //pin number used for tuning down in freq
...
long Band40mtop = 704000000; //Highest frequency 40m Band
...
long  Currentbandtop, Currentbandbottom; //Kepps tracks of the bandedges of the current selected band
...
void loop()
{
 ...
 // Serial.print("Buttonup=");
  ...
  //Serial.print("Buttondn=");
  //Serial.println(buttondn);
  ...
  if (!buttonup || !buttondn) //either tune button pressed?
  {
    ...
    if (!buttonup) //tuning up in freq
    {
      ...
      if (freq > Currentbandtop) // If we reached band edge, roll over
      {
        ...
        Serial.println("Rolled over to bottom");
      }
    ...
    if (!buttondn) //tuning down in freq
    {
      ...
      {
        freq = Currentbandtop ;
        Serial.println("Rolled over to Top");
      }
      si5351.set_freq(freq, SI5351_CLK0);  //set vfo freq
      si5351.set_freq(freq, SI5351_CLK1);
    }
